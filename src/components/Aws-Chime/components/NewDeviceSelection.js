...
let canHear = null;
function NewDeviceSelection(props) {
  ...
  const doGetStoredDevices = () => getLocalStorage('chimeDevices') || {};
  const deviceSelection = async () => {
    const chimeDevices = doGetStoredDevices();
    const publishAudio = typeof chimeDevices?.isMicEnabled === 'boolean' ? chimeDevices?.isMicEnabled : true;
    const publishVideo = typeof chimeDevices?.isCamEnabled === 'boolean' ? chimeDevices?.isCamEnabled : true;
    ...
    //   (await chime.meetingReadinessChecker.checkAudioInput(chime.currentAudioInputDevice));
    // setAudioStatus(status);
    if (publishAudio) {
  ...
  const handleCameraChange = async (selectedDevice) => {
    setVideoPreviewStatus('Loading');
    const device = selectedDevice?.value || 'OFF';
    if (device === 'OFF') {
  ...
  useEffect(() => {
    if (!chime.audioVideo || !videoPreviewElement.current) return null;
    return async () => {
  ...
  }, [chime.audioVideo, videoPreviewElement.current]);
      ...
      <div id="flow-devices" className="flow">
        <div className="">
          <form id="form-devices">
            <div>
              ...
              <div>
                ...
                <AESelect
                  ...
                  downArrow="selectaudioDownArrow"
            ...
            <div className="mt-3 voice-text ">
              {/* <div
                    className="col-sm-3 cursor"
                    onClick={() => {
                      audioInputPreview(chime.currentAudioInputDevice);
                    }}
                  >
                    <div className="test-voice w-100">
                      <p className="d-block text-center">Test</p>
                    </div>
                  </div> */}
              <div>
            ...
            <div className="mt-3 d-flex voice-text">
              <div
                className="col-sm-3 p-l-0 cursor"
                onClick={() => {
                  audioTest();
                }}
              >
            ...
            <div className="d-flex video-cam-preview">
              <div className="col-sm-8 p-l-0">
              ...
              <div className="col-sm-4 text-right d-sm-block video-preview p-0">
            ...
            <div className="mt-0">
              <div>
                ...
                <div className="mt-3">
                  <AELabel labelClass="title-text" subHeader={t('VideoQuality')} htmlFor="camera" id="camera_label" />
                  <AESelect
                ...
                <div className="margin-top-mobile">
                  <AEButton
                    ...
                    onClick={async () => {
                      ...
                      if (videoPreviewElement?.current) {
                        await chime?.audioVideo?.stopVideoPreviewForVideoInput(videoPreviewElement.current);
                      }
                      props.joinMeeting();
                    }}
...
export default NewDeviceSelection;
